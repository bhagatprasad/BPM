...
/*namespace BPM.Web.InventoryManagement.API.Services
{
    public class StockMovementService : IStockMovementService
    {
        ...
        public StockMovementService(IStockMovementRepository repository, ApplicationDbContext context, ILogger<StockMovementService> logger)
        {
        ...
        public async Task<StockMovementResponseDto> CreateAsync(StockMovementCreateDto dto)
        {
            try
            {
                ...
                var drugExists = await _context.Drugs.AnyAsync(x => x.DrugId == dto.DrugId);

                if (!drugExists)
                {
                    _logger.LogWarning("Drug not found: {DrugId}", dto.DrugId);
                    throw new KeyNotFoundException("Drug not found.");
                }

                var packagingExists = await _context.DrugPackagings.AnyAsync(x => x.PackagingId == dto.PackagingId);

                if (!packagingExists)
                {
                    _logger.LogWarning("Packaging not found: {PackagingId}", dto.PackagingId);
                    throw new KeyNotFoundException("Packaging not found.");
                }

                var warehouseExists = await _context.Warehouses.AnyAsync(x => x.Id == dto.WarehouseId);

                if (!warehouseExists)
                {
                    _logger.LogWarning("Warehouse not found: {WarehouseId}", dto.WarehouseId);
